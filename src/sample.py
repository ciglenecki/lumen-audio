...
def parse():
    ...
    parser.add_argument('--load_path', type=str, required=True, help='The path to load data that will be sampled')
    parser.add_argument('--save_path', type=str, required=True, help='The path to save sampled data .csv file')
    parser.add_argument('--n', type=int, required=True, help='The number of examples in total that will be sampled')
    parser.add_argument('--info_filename', type=str, default='info.csv', help='The filename for the information dataframe')
    parser.add_argument('--seed', type=int, default=42, help='The seed used for sampling (important for reproducibility)')
...
def move_uniform_sample(save_path, load_path, info_filename, n):
    ...
    for i in instruments:
        ...
        num_for_class = np.random.choice(
            len(all_ex_for_class), 
            size=n//len(instruments), 
            replace=False
        )
