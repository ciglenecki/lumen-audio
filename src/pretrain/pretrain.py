...
def parse():
    ...
    parser.add_argument("--shuffle_prob", type=float, default=0.3, help="The percentage of patches that will be shuffled within the same image.")
    parser.add_argument("--random_prob", type=float, default=0.3, help="The percentage of patches that will be replaced with random patches from different spectrograms.")
    parser.add_argument("--black_prob", type=float, default=0.1, help="The percentage of patches that will be blacked out. This isn't used to calculate the loss, it just makes the problem harder.")
    parser.add_argument("--width_div", type=int, default=4, help="The number of chunks in width dimension.")
    parser.add_argument("--height_div", type=int, default=4, help="The number of chunks in height dimension.")
...
def split_img_into_tiles(img, width_divisor, height_divisor):
    ...
    for j in range(height_divisor):
        for i in range(width_divisor):
            chunk_ij = img[:, :, j*chunk_height:(j+1)*chunk_height, i*chunk_width:(i+1)*chunk_width]
...
def get_model_for_pretraining(model, width_divisor, height_divisor):
    ...
    model.heads = nn.ModuleList(
        [nn.Linear(in_features=last_dim, out_features=3) for _ in range(width_divisor * height_divisor)]
    )
        
    return model

def create_corrupted_patches_and_labels(features_tiles, features_random_tiles, shuffle_prob, random_prob, black_prob):
    ...
    for b in range(features_tiles.shape[1]):
        ...
        dist_tensor[:, 0] = shuffle_prob # label 0 -> randomly shuffled within same exapmle
        dist_tensor[:, 1] = random_prob  # label 1 -> randomly taken from different example
        dist_tensor[:, 2] = 1 - (shuffle_prob + random_prob + black_prob) # label 2 -> original
        ...
        sample_shuffle_rand_perm_indices = sample_shuffle_indices[torch.randperm(len(sample_shuffle_indices))]
        
        #print(sample_shuffle_indices)
        #print(sample_shuffle_rand_perm_indices)
        
        features_tiles[sample_shuffle_indices] = features_tiles[sample_shuffle_rand_perm_indices]
        # in case there are indices where shuffling didnt occur, these stay original
        same_index_mask = sample_shuffle_indices[torch.nonzero(sample_shuffle_indices == sample_shuffle_rand_perm_indices)]
        ...
        features_tiles[sample_random_indices] = features_random_tiles[sample_random_indices]
...
def save_models(model, epoch, path, save_heads):
    ...
    if save_heads:
        for i, h in enumerate(model.heads):
            save_path_heads = os.path.join(path, f"epoch_{epoch}", f"epoch_{epoch}_head_{i}.pt")
...
if __name__=="__main__":
    ...
    num_training_steps = len(train_dataloader) * config.epochs // pl_args.accumulate_grad_batches
    ...
    full_save_path = os.path.join(f"{args.save_path}", f"{config.model.value}_{config.audio_transform.value}")
    ...
    for e in range(config.epochs):
        for batch in tqdm(train_dataloader):
            ...
            features_random_tiles = split_img_into_tiles(
                features_random, width_divisor=args.width_div, height_divisor=args.height_div
            )
            
            inputs, labels = create_corrupted_patches_and_labels(
                features_tiles, features_random_tiles,
            ...
            step += 1
            if pl_args.accumulate_grad_batches is None or step % pl_args.accumulate_grad_batches == 0:
                optimizer.zero_grad()
                optimizer.step()
                scheduler.step()
