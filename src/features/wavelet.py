...
import numpy as np
from scipy.signal import ricker, gausspulse
...
class DerivativeGaussWavelet(GeneralWaveletForm):
    ...
    def __call__(self,scale):
        return -self.t*np.exp(-0.5*self.t**2 /scale**2)/(np.sqrt(2*np.pi))/scale**2
...
class WaveletConv(nn.Module):
    ...
    def __init__(
        ...
        self.stride = 512#int(2*self.sampling_rate/129)#int(stride_in_time * sampling_rate)
        self.wavelet_form = wavelet_form(n_pts=self.kernel_size, time_res=self.time_resolution)

        # Create the wavelet filters
        self.filters = nn.Parameter(
            torch.zeros((self.n_scales, 1, self.kernel_size), dtype=torch.float32)
        )
        scales = torch.logspace(np.log10(self.min_s), np.log10(self.max_s), self.n_scales)
        self.filters_data = []
        for scale in scales:
        ...
        self.conv = nn.Conv1d(1, self.n_scales, kernel_size=self.kernel_size, stride=self.stride, bias=False)
