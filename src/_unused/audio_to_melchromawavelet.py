...
import _unused.audio_to_wavelet as audio_to_wavelet
...
class MelChroWavelet(AudioTransformBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.wavelet = audio_to_wavelet.WaveletConv(
            sampling_rate=self.sampling_rate, n_scales=self.n_mels
        )

    def __call__(self, audio):
        mel = librosa.feature.melspectrogram(y=audio, n_mels=128, sr=self.sampling_rate)
        mel = librosa.amplitude_to_db(mel)
        mel = (mel - mel.mean()) / mel.std()
        mel = torch.tensor(mel)  # .unsqueeze(0)
        chroma = librosa.feature.chroma_stft(
            y=audio, n_chroma=self.n_mels, sr=self.sampling_rate
        )
        ...
        scalogram = nn.functional.pad(scalogram, pad=(0, diff))
